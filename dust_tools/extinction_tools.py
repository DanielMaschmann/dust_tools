import numpy as np
from dust_extinction.parameter_averages import CCM89, F99, G23  # , F04, GCC09, F19
...
from phangs_data_access import helper_func, sample_access
...
class ExtinctionTools:
    ...
    # @staticmethod
    # def get_coordinate_gal_ext_evb(ra, dec, rad_deg, method='SandF'):
    #     """
    ...
    #     print(gal_ebv_table.colnames)
    #
    #     print(gal_ebv_table['ext SandF ref'])
    #     print(gal_ebv_table['ext SandF mean'])
    #     print(gal_ebv_table['ext SFD mean'])
    #     print(gal_ebv_table['ext SFD ref'])
    #     exit()
    #
    ...
    def get_target_gal_ext_ebv(target, method='SandF', ebv_estimator='mean', rad_deg=None):
        ...
        return ExtinctionTools.get_coord_gal_ext_evb(ra=ra_target, dec=dec_target, rad_deg=rad_deg, method=method,
    ...
    def get_gal_ext_at_wave(ra, dec, wave_mu, rad_deg=None, method='SandF', ebv_estimator='mean', ext_law='F99', r_v=3.1):
        gal_ext_ebv = ExtinctionTools.get_coord_gal_ext_evb(ra=ra, dec=dec, method=method, ebv_estimator=ebv_estimator,
                                                      rad_deg=rad_deg)
    ...
    def get_gal_ext(ra, dec, wave_mu, rad_deg=None, method='SandF', ebv_estimator='mean', ext_law='F99', r_v=3.1):
        gal_ext_ebv = ExtinctionTools.get_coord_gal_ext_evb(ra=ra, dec=dec, rad_deg=rad_deg, method=method,
                                                      ebv_estimator=ebv_estimator)
    ...
    def get_target_gal_ext_band(target, obs, band, rad_deg=None, method='SandF', ebv_estimator='mean', ext_law='F99',
        ...
        gal_ext_ebv = ExtinctionTools.get_target_gal_ext_ebv(
    ...
    @staticmethod
    def band_ext_g23_ebv(wave, ebv, r_v=3.1):

        model_g23 = G23(Rv=r_v)
        reddening = model_g23(wave * u.micron) * r_v
    ...
    @staticmethod
    def mag_ext2ebv(mag, wave, r_v=3.1):
        model_ccm89 = CCM89(Rv=r_v)
        return mag / (model_ccm89(wave * u.micron) * r_v)
    ...
    @staticmethod
    def color_ext_g23_av(wave1, wave2, av, r_v=3.1):

        model_g23 = G23(Rv=r_v)
        reddening1 = model_g23(wave1*u.micron) * r_v
        reddening2 = model_g23(wave2*u.micron) * r_v

        wave_v = 5388.55 * 1e-4
        reddening_v = model_g23(wave_v*u.micron) * r_v
